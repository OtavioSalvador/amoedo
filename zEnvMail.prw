...
User Function zEnvMail(cPara, cAssunto, cCorpo, aAnexos, lMostraLog, lUsaTLS)
    ...
    Local oSrv         := Nil
    Local nRet         := 0
    Local cFrom        := "otavio.salvador@amoedo.com.br"  //Alltrim(SuperGetMV("MV_RELACNT",,"otavio.salvador@amoedo.com.br"))
    Local cUser        := SubStr(cFrom, 1, At('@', cFrom)-1)
    Local cPass        := "Falange_11"  //Alltrim(SuperGetMV("MV_RELPSW",,"Falange_11"))
    Local cSrvFull     := "smtp-rec.amoedo.com.br:587"   //Alltrim(SuperGetMV("MV_RELSERV",,"smtp-rec.amoedo.com.br:587"))
    Local cServer      := Iif(':' $ cSrvFull, SubStr(cSrvFull, 1, At(':', cSrvFull)-1), cSrvFull)
    Local nPort        := Iif(':' $ cSrvFull, Val(SubStr(cSrvFull, At(':', cSrvFull)+1, Len(cSrvFull))), 587)
    Local nTimeOut     := 120  //SuperGetMV("MV_RELTIME",,120)
    Local cLog         := ""
    Default cPara      := ""
    Default cAssunto   := ""
    Default cCorpo     := ""
